...
| 问题 | 产生原因 | 解决方案 |
|------|----------|----------|
| 缓存穿透 | 查询一个根本不存在的数据，缓存和数据库中都没有，导致每次请求都直接打到数据库 | 1. 参数校验 2. 布隆过滤器 3. 缓存空值 |
| 缓存击穿 | 某个热点key在过期瞬间，大量并发请求直接打到数据库 | 1. 永不过期+异步更新 2. 互斥锁 3. 逻辑过期 |
| 缓存雪崩 | 大量缓存key在同一时间过期，或缓存服务宕机，导致所有请求直接打到数据库 | 1. 随机过期时间 2. 多级缓存 3. 高可用架构 |
