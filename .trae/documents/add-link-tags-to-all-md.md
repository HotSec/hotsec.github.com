# 计划：为 ALL.md 每个 details 块添加 link 标签

## 目标

在 `src/ALL.md` 的每个 `<details><summary>xxx</summary>` 块中，在 `</details>` 之前添加 `<link>` 标签，指向 `src/mybook/` 中对应的目录或文件。
...
逐段修改 ALL.md，为每个 `<details>` 块添加 `<link>` 标签。共约 111 个 details 块。
