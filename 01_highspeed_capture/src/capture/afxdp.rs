...
const SOL_XDP: i32 = 283; // Fixed value
const XDP_MMAP_OFFSETS: i32 = 1;
const XDP_RX_RING: i32 = 2;
const XDP_TX_RING: i32 = 3;
const XDP_UMEM_REG: i32 = 4; // Correct from header!
const XDP_UMEM_FILL_RING: i32 = 5; // Correct!
const XDP_UMEM_COMPLETION_RING: i32 = 6; // Correct!
const XDP_STATISTICS: i32 = 7;
const XDP_OPTIONS: i32 = 8; // Correct from header!
const XDP_USE_NEED_WAKEUP: u32 = 8; // Keep as is (1<<3)
...
impl XskSocket {
    pub fn create(
        ...
        unsafe {
            let ret = libc::setsockopt(
                ...
                &rx_ring_desc_count as *const _ as *const libc::c_void,
    ...
    let rx_mmap = Self::mmap_ring_fd(xsk_fd, XDP_PGOFF_RX_RING, rx_ring_size_bytes)?;
        ...
        let comp_ring = unsafe {
            Ring::new(comp_mmap, offsets.cr.producer, offsets.cr.consumer, offsets.cr.desc, offsets.cr.flags, comp_ring_desc_count)
        };
        let rx_ring = unsafe {
            Ring::new(rx_mmap, offsets.rx.producer, offsets.rx.consumer, offsets.rx.desc, offsets.rx.flags, rx_ring_desc_count)
        };
        ...
        unsafe {
            let ret = libc::setsockopt(
                ...
                &(XDP_USE_NEED_WAKEUP as u32) as *const _ as *const libc::c_void,
                std::mem::size_of::<u32>() as libc::socklen_t,
            );
            if ret != 0 {
                let err = io::Error::last_os_error();
